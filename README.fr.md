...
Cette extension privilégie une installation autonome et un comportement
reproductible : le moteur Palantir Java Format et son worker sont inclus dans le
VSIX. Après l’installation, seul un JDK 17 ou ultérieur est nécessaire.

Les extensions Java fondées sur Palantir Java Format n’adoptent pas toutes le
même modèle d’exécution. Voici les différences importantes pour choisir en
connaissance de cause :

| Critère | Cette extension | Autres approches courantes |
| --- | --- | --- |
| Installation du moteur | Inclus dans le VSIX | Clone d’un dépôt, JAR ou exécutable à fournir, ou téléchargement séparé |
| Outils requis à l’usage | JDK 17+ uniquement | Peut nécessiter Gradle, un dépôt local ou une configuration de chemin |
| Version du formateur | Version fixe, affichée et contrôlée au build | Peut dépendre de l’installation locale ou d’un téléchargement |
| Exécution | Worker Java local persistant | Nouveau processus par formatage ou daemon externe, selon l’extension |
| Communications | `stdin`/`stdout`, sans port réseau | Dépend de l’implémentation |
| Traitement du code | Aucun appel réseau pendant le formatage | Dépend du mode d’installation et d’exécution |
| Imports et chaînes longues | Pris en charge par l’API publique complète de Palantir | Fonctionnalités parfois exposées séparément ou configurables |

Ce choix est particulièrement adapté aux équipes qui veulent le même résultat
sur chaque poste, sans gérer elles-mêmes le binaire du formateur. En contrepartie,
la version de Palantir Java Format évolue avec les mises à jour de l’extension et
le formatage porte actuellement sur le document complet, pas sur une sélection.
